// src/Main.js
import { world as world7, system as system4, ItemStack as ItemStack3, EnchantmentTypes as EnchantmentTypes3, Dimension as Dimension2, EntityEquippableComponent, EquipmentSlot } from "@minecraft/server";
...
// src/classes/Enchantments.js
import { EnchantmentTypes as EnchantmentTypes2, ItemEnchantableComponent } from "@minecraft/server";
var Enchantments2 = class {
  ...
  static setEnchantName(itemStack, enchantName, level) {
    const eCompo = itemStack.getComponent("minecraft:enchantments");
    let enchantmentType = EnchantmentTypes2.get(enchantName);
    if (enchantmentType.maxLevel < level) throw `Enchantment level ${level} too high!`;
    let enchantment = { level, type: enchantmentType.id };
...
var sharedWorldModel = new WorldModel();
var WorldManager_default = sharedWorldModel;

// src/CustomListeners.js
var PlayerHoldingBackpack = class {
  constructor() {
    this.running = true;
  }
  subscribe(method) {
    this.run = system2.runInterval(() => {
      let index = 0;
      let allPlayers = WorldManager_default.getAllPlayers();
      for (let player of allPlayers) {
        if (!player.isValid) {
          WorldManager_default.removePlayerIndex(index);
          break;
        }
        index++;
  ...
  unsubscribe() {
    this.running = false;
    system2.clearRun(this.run);
  }
};
var BackpackClosing = class {
  constructor() {
    this.running = true;
  }
  subscribe(method) {
    this.run = system2.runInterval(() => {
      const backpackEntities = WorldManager_default.getBackpacksWithTag("close");
...
function checkForNearbyPortal(Dimension3, Location) {
  const newLocation = Location;
  newLocation.y += 1;
  try {
    const currentPositionBlock = Dimension3.getBlock(newLocation);
    const blockList = [
...
var portalUtils_default = {
  checkForNearbyPortal
};

// src/backpackUtils.js
import { world as world6, EntityQueryOptions, Container as Container2, Entity, ItemStack as ItemStack2, Player, system as system3 } from "@minecraft/server";

// src/itemUtils.js
import { Container, Dimension, ItemStack, world as world5 } from "@minecraft/server";

// src/debugMsg.js
import { world as world4 } from "@minecraft/server";
function sendDebugMsg(msg) {
  if (WorldManager_default.backpackDebugging) {
    world4.sendMessage(`\xA7a\xA7l[Backpack Debug] \xA7r${msg}`);
  }
}
...
function formatItemForLore(item) {
  const itemName = item.typeId.split(":")[1].split("_").map((str) => str.charAt(0).toUpperCase() + str.slice(1)).join(" ");
  return `\xA77${itemName} x${item.amount}`;
}
...
function getItemProperties(item) {
  const itemInterface = {
    id: item.typeId,
    name: item.nameTag || void 0,
    amount: item.amount,
    lore: item.getLore().length > 0 ? item.getLore() : void 0,
    durability: item.hasComponent("durability") ? item.getComponent("durability").damage : void 0,
    enchant: []
  };
  const enchantList = Enchantments2.getEnchants(item);
...
// src/backpackUtils.js
var DimensionList = [MinecraftDimensionTypes.Overworld, MinecraftDimensionTypes.Nether, MinecraftDimensionTypes.TheEnd];
function getBackpackEntityByQuery(query) {
  let dimensions = DimensionList;
  let entityList = [];
  for (let dimension of dimensions) {
    const bpEntities = world6.getDimension(dimension).getEntities(query);
...
function recordBackpackItems(id, inventory, bpEntity) {
  ...
  const lore = [`${id}`];
  for (let slot2 = 0; slot2 < inventory.size; slot2++) {
    const item = inventory.getItem(slot2);
    if (!item) continue;
    if (itemUtils_default.isForbiddenItem(item)) {
      bpEntity.dimension.spawnItem(item, bpEntity.location);
      inventory.setItem(slot2, void 0);
      continue;
    }
    backupItemList.push(itemUtils_default.getItemProperties(item));
    if (count < 5) {
      lore.push(itemUtils_default.formatItemForLore(item));
    } else {
      countOverflow++;
    }
    count++;
  ...
  if (countOverflow > 0) {
    lore.push(`\xA77and ${countOverflow} more...`);
  }
  world6.setDynamicProperty(`${id}`, JSON.stringify(backupItemList));
  return lore;
}
function openBackpack(player, bpItem) {
  ...
  if (bpOldEntities.length === 0) {
    player.sendMessage(`\xA7c[Backpack+] Unable to retrieve backpack items.`);
    player.sendMessage(`\xA7c[Backpack+] Will attempt to recover items.`);
...
function recoverBackpackItems(player, id) {
  const itemProperties = world6.getDynamicProperty(id);
  if (itemProperties) {
    ...
    for (const itemProperty of itemPropertiesParse) {
      const item = new ItemStack2(itemProperty.id, itemProperty.amount);
    ...
    player.sendMessage(`\xA7e[Backpack+] Items successfully recovered.`);
    return [bpEntity];
  } else {
    player.sendMessage(`\xA7c[Backpack+] Unable to find recovery backup.`);
    return false;
  }
}
function transferBackpackItems(sourceInventory, targetInventory) {
  for (let index = 0; index < sourceInventory.size; index++) {
    ...
    if (item) {
      ...
      sourceInventory.setItem(index, void 0);
    }
  }
}
var backpackUtils_default = {
...
// src/Main.js
var OVERWORLD_Y_OFFSET = -64;
var NETHER_Y_OFFSET = 0;
var END_Y_OFFSET = 0;
var BACKPACK_TELEPORT_HEIGHT = 1.5;
var holdingBackpackEvent = new PlayerHoldingBackpack();
var backpackClosingEvent = new BackpackClosing();
world7.afterEvents.worldLoad.subscribe(() => {
  system4.runTimeout(() => {
    BackpackMain();
  }, 20);
});
world7.afterEvents.playerJoin.subscribe(({ playerId }) => {
  system4.runTimeout(() => {
    const player = world7.getEntity(playerId);
    ...
    if (heldItem?.typeId.includes("bps:backpack")) {
      WorldManager_default.addPlayer(player);
    }
  }, 10);
});
world7.afterEvents.playerLeave.subscribe(({ playerName }) => {
  WorldManager_default.removePlayer(playerName);
});
function BackpackMain() {
  world7.sendMessage(`\xA7l\xA7a[Backpack Plus] Addon Active!`);
  console.warn(`\xA7l\xA7a[Backpack Plus] Addon Active!`);
  WorldManager_default.updateAllBackpacks();
  world7.afterEvents.playerInventoryItemChange.subscribe(({ slot: slot2, beforeItemStack, itemStack, player }) => {
    const mainHandSlot = player.selectedSlotIndex;
    if (!itemStack) return;
    if (mainHandSlot == slot2) {
      if (itemStack.typeId.includes("bps:backpack")) {
        const lore = itemStack.getLore();
        sendDebugMsg(`Backpack got added in hotbar main hand slot.`);
        const propertyInterface = {
          item: itemStack ? itemStack.typeId : void 0,
          slot: mainHandSlot,
          id: itemStack && lore.length > 0 ? lore[0] : void 0
        };
        ...
        if (itemStack?.typeId.includes("bps:backpack")) {
          ...
          WorldManager_default.removePlayer(player.name);
        }
      }
    }
  });
  world7.afterEvents.playerHotbarSelectedSlotChange.subscribe(({ player, itemStack: itemHolding }) => {
    const currentHeldItemStatusProperty = player.getDynamicProperty(`playerHeldItemStatus`);
    const inventory = player.getComponent("inventory").container;
    const selectedSlot = player.selectedSlotIndex;
    const propertyInterface = {
      item: itemHolding ? itemHolding.typeId : void 0,
      slot: selectedSlot,
      id: itemHolding && itemHolding.getLore().length > 0 ? itemHolding.getLore()[0] : void 0
    };
    const newPropertyJson = JSON.stringify(propertyInterface);
  ...
  holdingBackpackEvent.subscribe(({ itemHolding, player }) => {
    ...
    if (backpackID != void 0) {
      let bpEntityQuery = { tags: [backpackID] };
      let bpEntities = backpackUtils_default.getBackpackEntityByQuery(bpEntityQuery);
      for (let bpEntity of bpEntities) {
        if (portalUtils_default.checkForNearbyPortal(dimension, location)) {
          backpackUtils_default.closeBackpack(bpEntity);
          continue;
        }
  ...
  backpackClosingEvent.subscribe(({ bpEntity }) => {
    ...
    let lore = backpackUtils_default.recordBackpackItems(bpEntity.getTags()[0], bpEntityInventory, bpEntity);
  ...
  system4.runInterval(() => {
    let bpEntities = backpackUtils_default.getBackpackEntityByQuery({ families: ["bps"] });
    for (let bpEntity of bpEntities) {
...
async function onChanged(player, playerInventory, propertyDataCurrent, propertyDataOld) {
  ...
  if (currentItem && currentItem.typeId.includes("bps:backpack")) {
    ...
    if (lore.length === 0) {
      ...
      const bpId = `bps_id:${(Date.now() + Math.floor(Math.random() * 1e4)).toString().slice(-4)}`;
      currentItem.setLore([bpId]);
      playerInventory.setItem(propertyDataCurrent.slot, currentItem);
      const bpEntity = player.dimension.spawnEntity("bps:container_entity_temp", player.location);
      bpEntity.triggerEvent(backpackUtils_default.getBackpackContainerSize(currentItem));
      bpEntity.addTag(bpId);
      bpEntity.nameTag = backpackUtils_default.getBackpackContainerName(currentItem);
      WorldManager_default.updateAllBackpacks();
    } else {
      sendDebugMsg(`Opening Old Backpack`);
      backpackUtils_default.handleBackpackEntities(player, oldBackpackId, (bpEntity) => backpackUtils_default.closeBackpack(bpEntity, player, propertyDataOld));
      backpackUtils_default.openBackpack(player, currentItem);
      WorldManager_default.updateAllBackpacks();
    }
  } else if (currentItem) {
    sendDebugMsg(`Handling Forbidden Items`);
    backpackUtils_default.handleBackpackEntities(player, oldBackpackId, (bpEntity) => {
      const bpEntityInv = bpEntity.getComponent("inventory").container;
      itemUtils_default.handleForbiddenItems(bpEntityInv, bpEntity.location, bpEntity.dimension);
      backpackUtils_default.closeBackpack(bpEntity, player, propertyDataOld);
    });

...
import { config, waystoneInfo } from "../interfaces"
...
const dimensionList: { [key: string]: string } = {"world": "minecraft:overworld", "nether": "minecraft:nether", "end": "minecraft:the_end"}
...
export const apiOrganize = new class apiOrganize {
  organize(list: waystoneInfo[], type: boolean){
    const waystones = list
    if(!type) return waystones.sort((a, z) => { return a.id.localeCompare(z.id)})
    return waystones.sort((a, z) => { return z.id.localeCompare(a.id)})
  }

  organizeDimension(player: Player, list: waystoneInfo[]){

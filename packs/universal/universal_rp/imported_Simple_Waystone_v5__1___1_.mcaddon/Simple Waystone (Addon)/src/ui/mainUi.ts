import { config, configForm, waystoneEdit, waystoneForm, waystoneInfo } from "../interfaces"
...
import { world, Player, Block, system } from "@minecraft/server"
...
export const colorDimension: { [key: string]: string } = {"minecraft:overworld": "§2", "minecraft:nether": "§4", "minecraft:the_end": "§u"}
const xpSprite = ["", " - \ue700", " - \ue701", " - \ue702"]

export const waystoneUi = new class waystoneUi {
  createPoint(player: Player, block: Block, edit?: waystoneEdit){
    ...
    if(!edit) form.toggle("ui.waystone.create.toggle")
    form.submitButton("ui.waystone.create.button")
    .show(player).then(r => {
      if(r.canceled) return
      const options = r.formValues as waystoneForm
      if(!options[0]) return player.onScreenDisplay.setActionBar({translate: "waystone.warning.failCreateWaystone"})
      if(options[0].replaceAll(/§./g, "").replaceAll("§", "").length < 1) return player.onScreenDisplay.setActionBar({translate: "waystone.warning.failCreateWaystone"})
  ...
  waystoneList(player: Player, waystone?: waystoneInfo){
    ...
    if(buttons.length < 1) return player.onScreenDisplay.setActionBar({translate: "waystone.warning.failFindWaystones"})
    ...
    form.show(player).then(r => {
      ...
      if(selected.id == waystone?.id) return player.onScreenDisplay.setActionBar({translate: "waystone.warning.tpCurrentWaystone"})
      if(player.level >= button.cost || player.getGameMode() == "creative"){
        if(!waystone) player.setDynamicProperty("ws:warpstoneCooldown", Math.floor(new Date().getTime() / 1000) +59)
        if(player.getGameMode() != "creative") player.addLevels(-button.cost)
  ...
  settingsMenu(player: Player, block: Block, waystone: waystoneEdit){
    const owner = waystone.value?.owner ?? player.id
    if(owner != player.id) return player.onScreenDisplay.setActionBar({translate: "waystone.warning.notOwner"})
    new ActionFormData()
    ...
    .show(player).then(r => {
      ...
      if(r.selection == 0) return this.settingsWaystone(player)
      return this.removeWaystones(player)
    })
  }

  settingsWaystone(player: Player){
    const config = JSON.parse(player.getDynamicProperty("ws:waystoneConfig") as string) as config
    new ModalFormData()
    .title("ui.waystone.settingsMenu.title")
    ...
    .show(player).then(r => {
      ...
      const options = r.formValues as configForm

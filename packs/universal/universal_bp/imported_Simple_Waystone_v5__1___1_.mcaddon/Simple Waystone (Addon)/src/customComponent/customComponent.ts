...
world.beforeEvents.worldInitialize.subscribe((data) => {
  data.blockComponentRegistry.registerCustomComponent("ws:waystone", {
    beforeOnPlayerPlace: e => {
      ...
      if(!replaceBlock.includes(up.typeId) || up.location.y >= dimension.heightRange.max) return e.cancel = true
      system.runTimeout(() => {
    ...
    onPlayerInteract: e => {
      ...
      if(!perm["ws:waystone_on"]) return waystoneUi.createPoint(player, block)
      const waystone = apiWaystoneInfo.findWaystone(block)
      if(!waystone){ apiWaystoneSpace.setOff(player, block); player.playSound("note.bass", {location: b.location}); return player.onScreenDisplay.setActionBar({translate: "waystone.warning.corrupted"}) }
      if(!apiWaystoneInfo.claimPoint(player, waystone.value)) return
      if(player.isSneaking) return waystoneUi.settingsMenu(player, block, waystone)
  ...
  data.itemComponentRegistry.registerCustomComponent("ws:warpstone", {
    onUse: ({source: player}) => {
      ...
      const cooldownEnd = player.getDynamicProperty("ws:warpstoneCooldown") as number ?? new Date().getTime() / 1000 -1
      if(date > cooldownEnd) return waystoneUi.waystoneList(player)

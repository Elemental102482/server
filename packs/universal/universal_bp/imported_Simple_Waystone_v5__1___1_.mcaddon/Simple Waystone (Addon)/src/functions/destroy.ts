...
import { pistonDirection } from "../interfaces"
...
world.afterEvents.playerBreakBlock.subscribe(({brokenBlockPermutation: blockPerm, block: b, dimension, itemStackBeforeBreak: item, player}) => {
  if(blockPerm.type.id.includes("ws:waystone_")){
    if(!item?.getComponent("enchantable")?.hasEnchantment("minecraft:silk_touch") && player.getGameMode() != "creative") dimension.spawnItem(blockPerm.getItemStack(), apiWaystoneSpace.setCenterVector(b.location))
    if(!blockPerm.getState("ws:waystone_on" as keyof BlockStateSuperset)) return
    const block = blockPerm.getState("ws:waystone" as keyof BlockStateSuperset) == 1 ? b : b.below(1)
    removeWaystone(dimension, block.location)
    player.playSound("beacon.deactivate", {location: block.location})
  }
})

const explosed: string[] = []

world.afterEvents.blockExplode.subscribe(({block: b, explodedBlockPermutation: blockPerm, dimension}) => {
  if(blockPerm.type.id.includes("ws:waystone_")){
    const newPos = apiWaystoneSpace.getRelativeVector(b.location, blockPerm.getState("ws:waystone" as keyof BlockStateSuperset) == 1 ? 1 : -1)
...
const pistonDirection: pistonDirection[] = [["y", -1], ["y", 1], ["z", 1], ["z", -1], ["x", 1], ["x", -1]]

world.afterEvents.pistonActivate.subscribe(({block, dimension, isExpanding, piston}) => {
  const increase = pistonDirection[block.permutation.getState("facing_direction") as number]
  ...
  system.runTimeout(() => {
    for(let pos of locations){
      try{
        ...
        if(!block.typeId.includes("ws:waystone_")) continue
...
export function removeWaystone(dimension: Dimension, pos: Vector3){
  ...
  for(const id of Object.keys(allWaystonePrivate)){
    ...
    if(waystoneIndex == -1) continue
    waystones.splice(waystoneIndex, 1)
    apiInfo.saveAllWaystones(id, waystones, "private")
  }
  const claimList: string[] = world.getDynamicPropertyIds().map(dyn => dyn.startsWith(`ws:${apiOrganize.sortPos(pos).replaceAll("\"", "'")}=`) ? dyn.replaceAll("\"", "'") : null).filter(value => value !== null)
